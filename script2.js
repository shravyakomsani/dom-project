 Data= [
    ...
    e.map((i)=>{
        ...
        <img src="${i.image}" class="card-img-top" alt="${i.name}" loading="lazy" width="100%" height="200" style="object-fit: cover;">

        <div class="card-body">
        <div class="d-flex justify-content-between">
         <h5 class="card-title">${i.name}</h5>
         <select class="dropdowncategory" onclick="showMealType(event)">
         ${i.mealType.map((dish)=>(
          ...
          <p class="card-text" title="${i.instructions}">
          ${i.instructions[0].length > 30 ? i.instructions[0].slice(0,30):i.instructions[0]}......</p>
          <a href="#" class="btn btn-primary">cook now</a>
    ...
    function SearchMeal(e){
      e.preventDefault()
     let needed= document.getElementById("input_search").value.toLowerCase();
     let filtered= Data.filter(receipe=>
      receipe.mealType.some(type=>type.toLowerCase()==needed) 
       )
      document.getElementById("box").innerHTML=""; 
      showall(filtered)
      }
   ...
   function updaterange(e){
    document.getElementById("change").textContent=e.target.value;

   }
  function productrange(e){
    let range=parseInt(e.target.value);
    document.getElementById("change").textContent=range;
   let filtered= Data.filter((i)=>i.caloriesPerServing<range);
   document.getElementById("box").innerHTML="";
   if(filtered.length==0){
    document.getElementById("newrange").innerHTML = "<p>No results found for this calorie range.</p>";
  }
   else{
    showall(filtered)
   }
